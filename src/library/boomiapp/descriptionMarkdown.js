const add_description_listener = (description) => {
  ...
  const converter = new showdown.Converter({ literalMidWordUnderscores: true }),
    renderMD = () => {
      ...
      var html = null;
      if (!description.querySelector(".bph-md"))
        html = `<p class="bph-md">${converter.makeHtml(text_value)}</p>`;
  ...
  let observer = new MutationObserver((event) => {
    if (
      !description.querySelector("p").classList.contains("no_display") &&
      !description.classList.contains("no_display")
    ) {
      renderMD();
      description
        .closest(".component_header")
        .querySelector(".links .bph-markdown-toggle")
  ...
  if (
    BoomiPlatform.description_markdown == "on" ||
    !BoomiPlatform.description_markdown
  ) {
  ...
  description
    .closest(".component_header")
    .querySelector(".links")
    .insertAdjacentHTML("beforeend", toggle_html);
};

const toggleMarkdown = (target) => {
  target
    .closest(".component_header")
    .querySelector(".description_panel")
    .classList.toggle("render-markdown");
  target
    .closest(".component_header")
    .querySelector(".description_panel > p")
    .classList.toggle("render-markdown");
};
