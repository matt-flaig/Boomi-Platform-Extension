...
function rendorQuickShapePopup(obj, offsetX, offsetY, clientX, clientY) {
        ...
        if (e.key === 'Escape') {
            ...
            if (e.key === 'Enter') {
                if (filter?.trim()) {
                    ...
                    if (option !== undefined) {
                        var shape = $(this).closest('.component_editor_panel').find(`.gwt-Label.base_shape_container_label:contains('${$(option).attr('data-locator')}')`)[0]
                        shape.parentElement.parentElement.dispatchEvent(new MouseEvent('mousedown'));
                        obj.dispatchEvent(new MouseEvent('mouseup', { clientX: clientX, clientY: clientY }));
                    }
                }
                $('.bpe-quickshape-popup').each(function () {$(this).remove();});
                return;
    ...
    $('.bpe-quickshape-shape').on('mousedown', function (e) {
        ...
        var shape = $(this).closest('.component_editor_panel').find(`.gwt-Label.base_shape_container_label:contains('${$(this).attr('data-locator')}')`)[0]
        shape.parentElement.parentElement.dispatchEvent(new MouseEvent('mousedown'));
        obj.dispatchEvent(new MouseEvent('mouseup', { clientX: clientX, clientY: clientY }));
        $('.bpe-quickshape-popup').each(function () {$(this).remove();});
    });
