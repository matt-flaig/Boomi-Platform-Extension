  ...
  setTimeout(() => {
    ...
    window.addEventListener("keydown", (event) => {
      ...
      if (
        ...
        if (BoomiPlatform.full_screen_shortcut_alt && !event.altKey)
          return false;
        if (BoomiPlatform.full_screen_shortcut_ctrl && !event.ctrlKey)
          return false;
        if (BoomiPlatform.full_screen_shortcut_shift && !event.shiftKey)
          return false;
        ...
        setTimeout(() => {
          ...
          document
            .getElementById(tempPopupbutton)
            .querySelectorAll(".gwt-Anchor")[0]
            .click();
        }, 1);
      }
    });
